...
import type { HandValue, Player, Pot, PotAward } from './types';

/** The slice of `Player` that pot construction actually needs. */
export type PotPlayer = Pick<Player, 'id' | 'totalCommitted' | 'status'>;
...
export function awardPots(
  pots: readonly Pot[],
  strengths: ReadonlyMap<string, HandValue | null>,
  oddChipOrder: readonly string[],
): PotAward[] {
  const seatRank = new Map(oddChipOrder.map((id, index) => [id, index]));
  const positionOf = (id: string): number => seatRank.get(id) ?? Number.MAX_SAFE_INTEGER;
  ...
  return pots.map((pot, potIndex) => {
    ...
    const claimants =
      contenders.length > 0 ? contenders : pot.eligiblePlayerIds.slice();
    ...
    if (contenders.length > 1) {
      winners = claimants.reduce<string[]>((best, id) => {
        if (best.length === 0) return [id];
        const cmp = compareHands(
          strengths.get(id) as HandValue,
          strengths.get(best[0]) as HandValue,
        );
        if (cmp > 0) return [id];
        if (cmp === 0) return [...best, id];
        return best;
      }, []);
    }
...
export function payoutsFromAwards(awards: readonly PotAward[]): Record<string, number> {
